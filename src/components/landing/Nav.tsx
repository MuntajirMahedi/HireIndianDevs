...
export const Nav = () => {
  ...
  return (
    <header
      className={`fixed left-1/2 z-50 w-full max-w-6xl -translate-x-1/2 px-4 transition-all duration-300 ${
        scrolled ? "top-2" : "top-4"
      }`}
    >
      <div className="relative overflow-hidden rounded-full border border-white/10 bg-ink/95 px-4 shadow-2xl backdrop-blur-md md:px-6">
        <div className="flex h-14 items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white transition-colors hover:bg-white/20 lg:hidden"
              onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div className="mt-2 animate-in fade-in slide-in-from-top-2 overflow-hidden rounded-3xl border border-white/10 bg-ink p-2 backdrop-blur-xl lg:hidden">
